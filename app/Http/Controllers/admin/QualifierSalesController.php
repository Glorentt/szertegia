...
class QualifierSalesController extends Controller
{
    public function index(){
        ...
        return view('admin.sales.qualifier.index',compact('users'));
    }
    ...
    public function add(Request $request){
        ...
        $sale->save();
        return redirect()->route('admin.sales.qualifier')->with('success','Guardado');
    }
    public function minus(Request $request){
        $sale= Sale::where('user_id','=',$request->id)->orderBy('created_at', 'desc')->first();
        if ( $sale->delete()) {
            return redirect()->route('admin.sales.qualifier')->with('success','Borrado');
        }else{
            return redirect()->route('admin.sales.qualifier')->with('error','hay pedo');
        }
